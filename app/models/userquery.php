...
class UserPresenterSet extends PresenterSet
{
    ...
    public function asSelectListOptions($selected = null)
    {
        ...
        foreach ($this as $user) {
            ...
            $selectedText = "";
            if ($selected && ($user->id == $selected || strpos($selected, "|{$user->id}|") !== false)) {
...
class CurrentUser
{
    ...
    public function canViewPost(PostModel $post)
    {
        return $this->canEditPost($post) || array_search($this->model()->id, explode("|", trim($post->rights, "|"))) !== false;
    }

    public function canViewFile(FileModel $file)
    {
        return $this->canEditFile($file) || array_search($this->model()->id, explode("|", trim($file->rights, "|"))) !== false;
    }
