...
class NotORM_Row extends NotORM_Abstract implements IteratorAggregate, ArrayAccess, Countable {
    ...
    function getTableName($camelCase = false) {
        if (!$camelCase)
            return $this->result->table;
        else
            return implode('', array_map(function($var) {
                        return ucfirst(strtolower($var));
                    }, explode('_', $this->result->table)));
    }

    function isOfTable($table) {
        return strtolower($this->getTableName()) == strtolower($table);
    }
     ...
     */
    function model() {
        $n = $this->getTableName(true) . "Model";
        if (!class_exists($n))
            return null;
        $m = $n::create($this);
        return $m;
